Textos, cursos y otros basado en mi propia experiencia.

# Programar
* Aprende a programar
* Empieza por python (recomendado)
* Lectura recomendada: https://argentinaenpython.com/quiero-aprender-python/aprenda-a-pensar-como-un-programador-con-python.pdf

# Matemáticas
* Cursos recomendados: https://www.complexityexplorer.org/courses
  * Functions and iterations
  * Vector and Matrix Algebra
  * Introduction to Differential Equations
  * Ordinary Differential Equations

# Machine Learning
* Recomendado ver este curso para la vida misma: Introduction to Complexity https://www.complexityexplorer.org/courses
* Libro recomendado: https://github.com/KB-WEB-DEVELOPMENT/programming-collective-intelligence
* Curso recomendado (un clásico): https://www.coursera.org/specializations/machine-learning-introduction
* Libro básico con explicaciones sencillas para adentrarse en el mundo, un poco antiguo pero para las bases todavía es útil: https://iaarbook.github.io/

## Otros Machine learning
* Hidden Markov Models: https://www.linkedin.com/feed/update/urn:li:activity:7216019892477554688?utm_source=share&utm_medium=member_desktop

# Deep Learning / Redes Neuronales
* Lectura recomendada: https://www.packtpub.com/en-us/product/neural-network-projects-with-python-9781789138900
* Lectura MUY recomendada: https://www.linkedin.com/posts/emilio-rodrigo-carreira-villalta-2a62aa250_documentaci%C3%B3n-y-explicaci%C3%B3n-del-c%C3%B3digo-activity-7224404707329339395-UmNv?utm_source=share&utm_medium=member_desktop
* Gradiente Descendiente fácil: https://www.linkedin.com/feed/update/urn:li:activity:7219046799943102465?utm_source=share&utm_medium=member_desktop

## Tópicos avanzados de NLP
### Transformers
* Guía rápida sobre Auto atención: https://www.linkedin.com/feed/update/urn:li:activity:7216495470561374208?utm_source=share&utm_medium=member_desktop
* Explicación fácil de Transformers usando matrices aleatorias: https://osanseviero.github.io/hackerllama/blog/posts/random_transformer/
* una implementación de Transformers y GPT usando numpy en sólo 60 líneas: https://jaykmody.com/blog/gpt-from-scratch
* Survey de los principales tipos y variantes de Transformers (no entra en Mamba ni nada de eso): https://www.linkedin.com/feed/update/urn:li:activity:7191004992185540609?utm_source=share&utm_medium=member_desktop
* Visualización de Transformer: https://poloclub.github.io/transformer-explainer/
* Visualización con explicación paso a paso en 3D muy buena: https://bbycroft.net/llm
* Videos (Recomiendo ver en orden desde más antiguo a más nuevo) con explicaciones sencillas: https://www.youtube.com/@algorithmicsimplicity/videos
* Otra playlist de divulgación con videos que van desde las redes neuronales hasta conceptos avanzados: https://www.youtube.com/playlist?list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi

### Agentes
* Curso de microsoft para principiantes, simple de entender: https://microsoft.github.io/ai-agents-for-beginners/translations/es/

## Tópicos avanzados de Visión
* AlexNET: https://www.linkedin.com/feed/update/urn:li:activity:7189186608426819585?utm_source=share&utm_medium=member_desktop
* Curso recomendado que usa INGENIERÍA DE CARACTERÍSTICAS (NO DEEP LEARNING) para clasificar imágenes: https://www.coursera.org/learn/clasificacion-imagenes hay otro de detección de objetos
* Curso fácil de entender (antes tienes que entender CNN y UNET) sobre redes de difusión: https://learn.deeplearning.ai/courses/diffusion-models/lesson/xb8aa/introduction

# Neurociencia computacional
Recomiendo verse varios cursos de Complexity Explorer y tener buenas bases de matemáticas y un poco de electrónica
* Curso recomendado, enfocado en modelos a diferentes escalas, presentar el área y obvio harta mates: https://www.coursera.org/learn/computational-neuroscience
* Otro curso muy recomendado, más enfocado en modelos basados en electrónica para las neuronas, sinápsis: https://www.coursera.org/learn/synapses
* Lectura recomendada (un poco dura y requiere conocimientos de matemáticas pero es bien explicativo, para leer con calma): https://boulderschool.yale.edu/sites/default/files/files/DayanAbbott.pdf

# Devops (pasar desde un código a un producto en producción para todo el público)
* Video muy recomendado de Docker: https://www.youtube.com/watch?v=9eTVZwMZJsA
* Otro video que te explica mejor la arquitectura de Docker: https://www.youtube.com/watch?v=gjRoNFopFig
* Curso recomendadísimo muy completo y detallado de Kubernetes: https://livro.descomplicandokubernetes.com.br/es/day-1/
* Video de git introductorio: https://www.youtube.com/watch?v=jGehuhFhtnE
* Video de git por dentro y cómo funciona (cómo está construido, los comandos de bajo nivel): https://www.youtube.com/watch?v=LjwR--_ZUt8

# Otras listas
* Un poco antigua pero muy buena todavía: https://machinelearningmastery.com/start-here/
* Recomendado para todo en general, artículos de todo IA, DL, ML, de todo un poco, tópicos avanzados: https://aman.ai/
* Tutorial de Clustering: https://dashee87.github.io/data%20science/general/Clustering-with-Scikit-with-GIFs/
